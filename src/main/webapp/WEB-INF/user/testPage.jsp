...
<head>
    ...
    <script>
        window.onload = function() {
            // dto.tutorcate_no 값을 JavaScript 변수로 설정
            var tutorcateNo = "${dto.tutorcate_no}";
            ...
            // 특정 tutorcate_no와 일치하는 id를 가진 form만 표시합니다.
            var targetForm = document.getElementById(tutorcateNo);
            if (targetForm) {
...
<form id="3" action="user/registerTest" method="post">
    ...
    <textarea> </textarea>
    <h4>사용자로부터 시간을 "hh:mm" 형식으로 입력받아, 입력된 시간을 초로 환산하여 출력하는 C# 프로그램을 작성하세요.</h4>
    <textarea> </textarea>
    <button type="submit">제출하기</button>
</form>
